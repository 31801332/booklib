...
public class ReaderManager {
	...
	public List<BeanReader> searchReader(String keyword,int readerTypeId)throws BaseException{
		...
		try {
			...
			sql+=" order by readerTypeId asc,readerid asc";
			java.sql.PreparedStatement pst=conn.prepareStatement(sql);
			if(keyword!=null && !"".equals(keyword)){
	...
	public List<BeanReader> searchPageReader(String keyword,int readerTypeId,int page,int count)throws BaseException{
		List<BeanReader> result=new ArrayList<BeanReader>();
		Connection conn=null;
		try {
			conn= DBUtil.getConnection();
...
//			pst.setDate(5, (Date) r.getCreateDate());
//			pst.setString(6,r.getCreatorUserId());
	...
	public BeanReader loadReader(String readerid) throws DbException {
		...
		try {
			...
			List<BeanBook> br = new ArrayList<BeanBook>();
			sql = "select barcode, bookname, pubid, price, state\n" +
					"from beanbook\n" +
					"where barcode in (\n" +
					"    select bookBarcode\n" +
					"    from beanbooklendrecord\n" +
					"    where readerid = '31801332' and returnDate is null\n" +
					")";
			pst=conn.prepareStatement(sql);
			pst.setString(1,readerid);
			rs = pst.executeQuery();
			while(rs.next()){
				BeanBook p = new BeanBook();
				p.setBarcode(rs.getString(1));
				p.setBookname(rs.getString(2));
				p.setPubid(rs.getString(3));
				p.setPrice(rs.getDouble(4));
				p.setState(rs.getString(5));
				br.add(p);
			}
			r.setNotReturnBook(br);
			pst.close();
			rs.close();
		} catch (SQLException e) {
			e.printStackTrace();
