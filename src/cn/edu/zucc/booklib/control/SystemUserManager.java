...
//		}
	...
	public BeanSystemUser loadUsersByUserid(String Userid) throws BaseException {
		BeanSystemUser result = new BeanSystemUser();
		Connection conn = null;
		try {
			conn = DBUtil.getConnection();
			String sql = "select userid,username,usertype,createDate from BeanSystemUser where Userid = '" + Userid + "'";
			java.sql.Statement st = conn.createStatement();
			java.sql.ResultSet rs = st.executeQuery(sql);
			if (rs.next()) {
				BeanSystemUser u = new BeanSystemUser();
	...
	public boolean modifyUserName(String userid, String username) throws BaseException {
		...
		try {
			...
			if (rs.next()) {
				...
				System.out.println("受影响的行：" + line);
				ans = line == 1 ? true : false;
			} else {
				System.out.println("未找到用户ID");
			}
			rs.close();
			pst.close();
			;
	...
	public static void main(String[] args) {
		...
		bsu.setUserid("jyy");
		bsu.setUsername("jyyname");
		bsu.setPwd("jyy");
		bsu.setUsertype("借阅员");
		bsu.setCreateDate(new Date());
